    ...
    def forward_mix(self, img, gt_label, remove_idx=-1):
        ...
        # applying hand-crafted sample mixup methods
            ...
            if cur_mode in ["mixup", "cutmix", "saliencymix", "smoothmix"]:
                img, gt_label, rand_index = self.static_mode[cur_mode](
        ...
        # mixup loss
        if self.is_IN:
            # cls head for training backbone and classification
            outs_mix = self.head(x_mix)
            IN = (outs_ori, outs_mix)
            losses, info = self.head.IN_loss(IN, gt_label, rand_index)
        else:
            outs = self.head(x_mix)
