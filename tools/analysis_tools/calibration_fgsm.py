...
from openmixup.utils import (get_root_logger, dist_forward_collect,
                             setup_multi_processes, nondist_forward_collect, traverse_replace, fgsm_nondist_forward_collect)
...
def fgsm_test(model, data_loader, head):
    model.eval()
    func = lambda **x: model(mode='test', **x)
    results = fgsm_nondist_forward_collect(func, data_loader,
                                      len(data_loader.dataset), head)
    return results
...
def parse_args():
    ...
    parser.add_argument(
        '--keys',
        type=str,
        default=None,   # choose calibration or fgsm
        help='the dir to save logs and models')
    parser.add_argument(
        '--head',
        type=str,
        default=None, # choose head : head0 or acc_mix
        help='choose head, [acc_mix_k, acc_one_k, acc_mix_q, acc_one_q] for automix, samix and adautomix and [head0] for mixups')
    parser.add_argument(
        '--save_name',
        type=str,
        default=None,
...
def main():
    ...
    if not distributed:
        ...
        if args.keys == 'fgsm':
            print("It`s FGSM(Fast Gradient Sign Method) compute adversarial robustness error")
            outputs = fgsm_test(model, data_loader, args.head)
        elif args.keys == 'ece' or 'calibration':
            print("It`s Calibration compute ECE")
            outputs = single_gpu_test(model, data_loader)
            result = dataset.ece_score(outputs[args.head], name=args.save_name)
            print("\n result：{}%".format(result * 100))
        else:
            print('\n False')
